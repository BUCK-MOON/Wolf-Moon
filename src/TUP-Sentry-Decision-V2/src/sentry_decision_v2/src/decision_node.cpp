...
namespace sentry
{
  ...
  bool RobotDecisionNode::readJsonFile()
  {
    ...
    for (int i = 0; i < int(arrayValue.size()); ++i)
    {
      ...
      temp->outpost_minHP = arrayValue[i]["opHPmin"].asInt();
      temp->outpost_maxHP = arrayValue[i]["opHPmax"].asInt();
  ...
  void RobotDecisionNode::run()
  {
    ...
    while (rclcpp::ok())
    {
      ...
      else
      {
        ...
        {
          RCLCPP_INFO(this->get_logger(), "Mission ABORTED, Retry!");
          mission_retrys++;
          if (mission_retrys == 3)
          {
            RCLCPP_INFO(this->get_logger(), "Mission Retry Times Over, Mission changed!");
            past_decision = currentDecision;
            blackboard.cleanUpMissions();
            mission_retrys = 0;
          }
  ...
  { // 返回值：shared_ptr<Decision_Warp>，指向权重最高的决策对象的智能指针。若无有效决策，返回nullptr。
