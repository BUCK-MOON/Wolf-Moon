...
{
    Detector::Detector() : logger_(rclcpp::get_logger("armor_detector"))
    {
    }
    ...
    bool Detector::initModel(std::string path)
    {
        ...
        RCLCPP_INFO_THROTTLE(logger_, steady_clock_, 50,
                             "\033[1m\033[36mStart initialize model...\033[0m");
        ...
        RCLCPP_INFO_THROTTLE(logger_, steady_clock_, 50,
                             "\033[1m\033[36mFinish initialize model...\033[0m");

        return true;
    }

    bool Detector::detect(cv::Mat &src, std::vector<ArmorObject> &objects)
    {
        if (src.empty())
        {
            return false;
