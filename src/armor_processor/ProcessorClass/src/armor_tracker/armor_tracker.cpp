...
namespace rm_auto_aim
{
    ...
    void Tracker::init(const Armors &armors_msg)
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "No matched armor init!");
            return;
        }
        ...
        updateArmorsNum(tracked_armor); // 查看装甲板id

        initEKF(tracked_armor);
        RCLCPP_WARN(rclcpp::get_logger("armor_tracker"), "Init EKF!");
    ...
    void Tracker::initEKF(const Armor &a)
    {
        ...
        dz = 0, another_r = r;
        target_state << xc, 0, yc, 0, za, 0, yaw, v, r;
    ...
    void Tracker::update(const Armors &armors_msg)
    {
        ...
        // RCLCPP_INFO(rclcpp::get_logger("armor_tracker"), "EKF predict");
        bool matched = false;
        ...
        if (!armors_msg.armors.empty())
        {
            ...
            std::vector<Armor> same_id_armor;
            int same_id_armors_count = 0;
            int min_position_diff_armor_id = 0;
            ...
            for (const auto &armor : armors_msg.armors)
            {
                ...
                {
                    same_id_armor.emplace_back(armor);
                    ...
                    if (position_diff < min_position_diff)
                    {
                        ...
                        min_position_diff_armor_id = same_id_armor.size();
            ...
            if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_)
            {
                ...
                if (tracked_armors_num == ArmorsNum::OUTPOST_3) // 前哨站普通更新
                {
                    target_state[7] = 0.8;
                    target_state[8] = 0.351;
                    ekf.setState(target_state);
                }
                // RCLCPP_INFO(rclcpp::get_logger("armor_tracker"), "EKF update");
                RCLCPP_WARN_THROTTLE(rclcpp::get_logger("armor_tracker"),steady_clock_, 500,"EKF update" );
            }
            else if (same_id_armors_count == 1 && yaw_diff > max_match_yaw_diff_)
            {
                // Matched armor not found, but there is only one armor with the same id
                // and yaw has jumped, take this case as the target is spinning and armor jumped
                handleArmorJump(same_id_armor[0]);
            }
            else
            {
    ...
    double Tracker::orientationToYaw(const geometry_msgs::msg::Quaternion &q)
    {
        ...
        yaw = last_yaw_ + angles::shortest_angular_distance(last_yaw_, yaw + CV_PI / 2);
        last_yaw_ = yaw;
        return yaw;
    }
