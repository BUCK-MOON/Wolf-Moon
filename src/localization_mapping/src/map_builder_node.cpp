...
namespace IG_LIO
{
    ...
    void MapBuilderNode::param_respond()
    {
...

        {
            ...
            local_rate_ = std::make_shared<rclcpp::Rate>(local_rate); // 生成分享指针
            loop_rate_lc_ = std::make_shared<rclcpp::Rate>(loop_rate_lc);
            loop_rate_l_ = std::make_shared<rclcpp::Rate>(loop_rate_l);
        }
        this->declare_parameter<double>("blind", 0.5);         // 盲点范围
        this->declare_parameter<double>("height_offset", 0.7); // 高度
    ...
    void MapBuilderNode::run()
    {
        ...
        if (current_state_.bg.norm() * 180.0 / M_PI > 4.0)
        {
