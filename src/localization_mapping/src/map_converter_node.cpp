#include "../include/map_converter_node.hpp"

namespace IG_LIO
{
    MapConverterNode::MapConverterNode(const rclcpp::NodeOptions &options)
    ...
    void MapConverterNode::param_respond()
    {
    ...
    void MapConverterNode::initPublishers()
    {
    ...
    void MapConverterNode::initSerivces()
    {
        CovertMap_Server = this->create_service<ig_lio_c_msgs::srv::CovertMap>("CovertMap",
                                                                               std::bind(&MapConverterNode::covertMapCallBack, this, _1, _2),
                                                                               rmw_qos_profile_services_default);
    }

    void MapConverterNode::init()
    {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr MapConverterNode::filterPointCloudByHeightRange(const pcl::PointCloud<pcl::PointXYZ>::Ptr &input_cloud)
    {
    ...
    grid_map::GridMap MapConverterNode::makeGridMapFromDepth(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_ptr, builtin_interfaces::msg::Time stamp)
    {
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    grid_map::GridMap MapConverterNode::makeGridMap(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_to_make)
    {
    ...
    void MapConverterNode::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        // RCLCPP_INFO_STREAM(this->get_logger(), GREEN << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << RESET);
    ...
    void MapConverterNode::publishGridMap(const grid_map::GridMap &gridMap_to_pub)
    {
    ...
    std::shared_ptr<nav_msgs::msg::OccupancyGrid> MapConverterNode::createOccupancyGridMsg(const grid_map::GridMap &gridMap)
    {
    ...
    void MapConverterNode::publishOccupancyGridMapMap(std::shared_ptr<nav_msgs::msg::OccupancyGrid> &occupancyGrid_to_pub)
    {
    ...
    void MapConverterNode::covertMapCallBack(const ig_lio_c_msgs::srv::CovertMap::Request::SharedPtr request,
                                                       const ig_lio_c_msgs::srv::CovertMap::Response::SharedPtr response)
    {
        // RCLCPP_INFO_STREAM(this->get_logger(), GREEN << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << RESET);
    ...
    void MapConverterNode::tryWriteMapToFile(
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<IG_LIO::MapConverterNode>();
    // RCLCPP_INFO_STREAM(node->get_logger(), GREEN << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << RESET);
    node->get_logger().set_level(rclcpp::Logger::Level::Error);
    // RCLCPP_INFO_STREAM(node->get_logger(), GREEN << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << RESET);
...
RCLCPP_COMPONENTS_REGISTER_NODE(IG_LIO::MapConverterNode)
