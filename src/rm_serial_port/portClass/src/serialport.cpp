...
namespace rm_auto_aim
{
    ...
    void Serialport::receiveData(auto_aim_interfaces::msg::Serial &serial_msg, auto_aim_interfaces::msg::Uart &uart_msg)
    {
        ...
        if (flag == 0xA5 || flag == 0xC5)
        {
            if (flag == 0xA5)
            {
                serial_msg.header.frame_id = "right";
            }
            else
            {
                serial_msg.header.frame_id = "left";
            }
        ...
        else if (flag == 0xB5)
        {
            ...
            uart_msg.header.stamp = now;
            uart_msg.header.frame_id = "uart";
            std::vector<ushort> hp; // 血量
            data_transform_->getHPInfo(&port_interface_->rdata[3], hp);
            uart_msg.outpost_hp = static_cast<unsigned int>(hp[0]);
            std::cout << "outpost_hp" << hp[0] << std::endl;
            uart_msg.sentry_hp = static_cast<unsigned int>(hp[1]);
            std::cout << "sentry_hp" << uart_msg.sentry_hp << std::endl;
            uart_msg.remain_time = static_cast<unsigned int>(hp[2]);
            std::cout << "remain_time" << uart_msg.remain_time << std::endl;
            uart_msg.x = static_cast<unsigned int>(hp[3]);
            uart_msg.y = static_cast<unsigned int>(hp[4]);
            std::cout << "x" << uart_msg.x << std::endl;
            std::cout << "x" << uart_msg.y << std::endl;
            uart_msg.game_stage = port_interface_->rdata[13];
            uart_msg.commd_keyboard = port_interface_->rdata[14];
            // std::cout<<"hp"<<uart_msg.remain_time<<std::endl;
        }
    }
    void Serialport::sendDataA(auto_aim_interfaces::msg::Gimbal::SharedPtr &gimbal_msg)
    {
        ...
        float pitch = (float)gimbal_msg->pitch;
        float yaw = (float)gimbal_msg->yaw;
